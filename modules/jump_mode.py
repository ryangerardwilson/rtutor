...
class JumpMode:
    ...
    def run(self, stdscr):
        # Typing mode for a single lesson, just once—no stupid loops
        stdscr.nodelay(True)
        ...
        while not lesson_finished:
            if need_redraw:
                ...
                title = f"Jump Mode: {self.sequencer_name} | {self.lesson.name}"
                try:
                ...
                total_count = sum(
                    len(processed_lines[i]) for i in range(len(lines)) if not is_skip[i]
                )
        ...
        # Done with the single run, no boom, just return True to signal completion
        return True
